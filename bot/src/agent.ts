...
function buildSystemPrompt(): string {
  ...
  return [
    ...
    "## Tools",
    "- planner_get_overview: today summary + open backlog count + next timed",
    "- planner_find_tasks: search (q), day=YYYY-MM-DD, open_backlog=true, or inbox=true",
    "- planner_create_task: inbox (omit day), all-day (day + is_all_day), timed (day + start_time)",
    "- planner_update_task / planner_complete_tasks / planner_reschedule: by task_id from find",
    "- Incomplete tasks NEVER auto-complete overnight. open_backlog = previously unticked dated tasks still on their original day.",
    "When modifying, find first then modify. Never guess task_id.",
    ...
    "- After every create/update/complete, confirm what was done",
